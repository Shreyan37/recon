...
use crate::classify::{ChangeSpan, SemanticMap};
use crate::{
    display::hunks::Hunk,
    parse::{
...
pub(crate) struct DiffResult {
    ...
    /// Changes that affect program behaviour, populated by `--summarize`.
    pub(crate) behavioral_changes: Vec<ChangeSpan>,
    /// Changes that are purely cosmetic (comments, whitespace), populated by
    /// `--summarize`.
    pub(crate) cosmetic_changes: Vec<ChangeSpan>,

    /// Per-position semantic classification for `--semantic-colors`.
    ///
    /// `None` when `--semantic-colors` was not requested, when
    /// `--color=never` is active, when `--summarize` is active (the maps
    /// would be unused), or when the file format does not support structural
    /// diffing (PlainText, TextFallback, Binary). In all those cases
    /// `apply_colors` falls back to the standard red/green novel highlighting.
    pub(crate) lhs_semantic_map: Option<SemanticMap>,
    pub(crate) rhs_semantic_map: Option<SemanticMap>,
}
